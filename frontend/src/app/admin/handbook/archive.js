...
export default function Archive({ open, close, selectedHandbookRow, onArchiveChange }) {
    const [archiveProgress, setArchiveProgress] = useState(false)
    const [archiveError, setArchiveError] = useState("")
    const [archivedHandbooks, setArchivedHandbooks] = useState([])
    const [loadingArchived, setLoadingArchived] = useState(false)
    const [openDropdownId, setOpenDropdownId] = useState(null)

    const isListView = !selectedHandbookRow
    ...
    const loadArchived = async () => {
        ...
        try {
            const res = await fetch("/api/admin/handbook?view=archived")
            const data = await res.json()
            if (data.success) {
                setArchivedHandbooks(data.data || [])
            } else {
                setArchiveError("Failed to load archived handbooks")
            }
        } catch (err) {
            setArchiveError("Error loading archived handbooks")
        } finally {
    ...
    const handleUnarchiveClick = async (handbook) => {
        try {
            const res = await fetch("/api/admin/handbook", {
                ...
                body: JSON.stringify({ handbook_id: handbook.handbook_id, action: "unarchive" })
            })
            const data = await res.json()
            if (data.success) {
    ...
    const submitArchive = async () => {
        ...
        try {
            const res = await fetch("/api/admin/handbook", {
                method: "PATCH",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ handbook_id: selectedHandbookRow.handbook_id, action: "archive" })
            })
            const data = await res.json()
            if (data.success) {
                ...
                <div className="flex flex-col h-full p-5 space-y-4">
                    <div className="flex mt-10 items-center justify-between mb-4">
                        <h2 className="text-lg font-semibold text-gray-800">
                            {isListView ? "Archived Handbooks" : "Archive Handbook"}
                        </h2>
                    ...
                    {isListView ? (
                        <div className="flex-1 overflow-y-auto">
                            ...
                            ) : archivedHandbooks.length === 0 ? (
                                <div className="text-center py-6 text-gray-500">No archived handbooks</div>
                            ) : (
                                                ...
                                                <div className="divide-y divide-gray-200">
                                                    {archivedHandbooks.map((handbook) => (
                                                        <div key={handbook.handbook_id} className="py-2 relative">
                                                            ...
                                                            >
                                                                <span className="truncate text-sm">{handbook.handbook_name}</span>
                                                            </div>
                        ...
                        <div className="flex-1 overflow-y-auto">
                            <p>Are you sure you want to archive <strong>{selectedHandbookRow?.handbook_name}</strong>?</p>
                        </div>
                    )}
