...
export default function HandbookCreate({ open, close, onHandbookCreated }) {
    ...
    const submit = async () => {
        ...
        try {
            ...
            if (result.success) {
                resetForm();
                close();
                if (onHandbookCreated) {
                    onHandbookCreated();
                }
            } else {
                setFileError(result.message || "Failed to upload handbook");
            }
        } catch (error) {
    ...
    return (
        <main className="z-500">
            <div className={`fixed inset-0 duration-300 ${open ? "opacity-100" : "opacity-0 pointer-events-none"}`} onClick={handleClose}></div>
                        ...
                        <div className="w-full flex-1 overflow-y-auto">
                            <div className="space-y-4 md:space-y-6">
                                <div className="space-y-2">
                                    <label htmlFor="handbookFile" className="block text-sm font-medium text-gray-700">Handbook Document (PDF only)</label>
                                    ...
                                    {fileName && (
                                        <div className="mt-2 p-2 bg-green-50 border border-green-200 rounded">
                                            <p className="text-sm text-green-700 flex items-center gap-2">
                                                ...
                                                Selected: {fileName} ({(handbookFile?.size / 1024 / 1024).toFixed(2)} MB)
                                            </p>
                                        </div>
                                    )}
                                    {fileError && <span className="text-red-600 text-sm mt-1 block">{fileError}</span>}
                                </div>
                            </div>
                        </div>
                    </div>
                    <button className="w-full bg-[#205781] text-white hover:bg-[#1a4a6b] transition duration-400 p-2 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed" onClick={submit} disabled={loading || !handbookFile}>
