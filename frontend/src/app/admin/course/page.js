...
export default function Course() {
    ...
    const fetchCourses = async () => {
        ...
        setError(null);
        try {
            const response = await fetch("/api/admin/course?view=active");
    ...
    const filteredData = courses.filter((row) => {
        const documentName = row.document_name ? String(row.document_name).toLowerCase() : "";
        const courseId = row.course_id ? String(row.course_id) : "";
        const searchLower = searchData.toLowerCase();
        return documentName.includes(searchLower) || courseId.includes(searchLower);
    });
    ...
    const handleViewDocument = async (courseId, documentName) => {
        try {
            const response = await fetch(`/api/admin/course?course_id=${courseId}`);
            const data = await response.json();
            if (data.success && data.data.course_document) {
                const byteCharacters = atob(data.data.course_document);
    ...
    const handleDownloadDocument = async (courseId, documentName) => {
        try {
            const response = await fetch(`/api/admin/course?course_id=${courseId}`);
            const data = await response.json();
            if (data.success && data.data.course_document) {
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-4">
                                        <div className="flex-1">
                                            ...
                                            <div className="relative">
                                                ...
                                                <input type="text" id="table-search" className="w-full h-11 pl-10 pr-4 text-sm text-gray-600 bg-white border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#205781]/20 focus:border-[#205781] transition-all duration-200" placeholder="Search by document name" value={searchData} onChange={search} />
                                            </div>
                                        </div>
                            ...
                            <div className="flex-1 overflow-x-auto overflow-y-auto">
                                {loading ? (
                                    ...
                                    <div className="flex flex-col justify-center items-center py-16 px-4">
                                        ...
                                        <p className="text-red-600 text-center">Error loading courses: {error}</p>
                                    </div>
                                ) : sortedData.length === 0 ? (
                                    <div className="flex flex-col justify-center items-center py-16">
                                        <svg className="w-12 h-12 text-gray-300 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4" /></svg>
                                        <p className="text-gray-500 text-center">{searchData ? "No matching courses found" : "No courses uploaded yet"}</p>
                                    </div>
                                ) : (
                                                ...
                                                <tbody className="divide-y divide-gray-200">
                                                    {sortedData.map((course, index) => (
                                                        <tr key={course.course_id ?? `course-${index}`} className="bg-white hover:bg-gray-50 transition-colors duration-150">
                                                            ...
                                                            <td className="px-6 py-4 text-center relative" data-dropdown={course.course_id}>
                                                                <div className="relative flex justify-center">
                                                                    <button onClick={() => toggleDropdown(course.course_id)} className="p-2 text-[#205781] hover:bg-[#205781]/10 rounded-lg transition-all duration-150">
                                                                    ...
                                                                    {openDropdownId === course.course_id && (
                                                                        <div className="origin-top-right absolute top-full right-0 mt-1 w-56 bg-white border border-slate-200 rounded-lg shadow-xl z-20">
                                                                            <ul className="py-1">
                                                                                <li>
                                                                                    <button onClick={() => handleDownloadDocument(course.course_id, course.document_name)} className="w-full text-left text-slate-800 hover:bg-slate-50 flex items-center p-3">
                                                                                ...
                                                                                <li>
                                                                                    <button onClick={() => handleArchive(course.course_id)} className="w-full text-left text-slate-800 hover:bg-slate-50 flex items-center p-3">
                                                                                        <div className="flex items-center justify-center bg-white border border-slate-200 rounded shadow-sm h-7 w-7 shrink-0 mr-3">
                                                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-4">
                                                                                                <path strokeLinecap="round" strokeLinejoin="round" d="m20.25 7.5-.625 10.632a2.25 2.25 0 0 1-2.247 2.118H6.622a2.25 2.25 0 0 1-2.247-2.118L3.75 7.5M10 11.25h4M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125Z" />
                                                                                            </svg>
                                                                                ...
                                                                                <li>
                                                                                    <button onClick={() => openUpdateModal(course)} className="w-full text-left text-slate-800 hover:bg-slate-50 flex items-center p-3">
                                                                                        <div className="flex items-center justify-center bg-white border border-slate-200 rounded shadow-sm h-7 w-7 shrink-0 mr-3">
                                                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-4">
