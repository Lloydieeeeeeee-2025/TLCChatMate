            ...
            <div className={`fixed top-0 right-0 h-full w-full md:w-[35%] bg-white border-l border-gray-200 transform transition-transform duration-300 ${open ? "translate-x-0" : "translate-x-full"}`}>
                <div className="flex flex-col h-full p-5 space-y-4">
                    <div className="flex items-center justify-between">
                        <h2 className="text-lg font-semibold text-gray-900">Update FAQ</h2>
                        <button onClick={handleClose} className="text-gray-600 hover:text-gray-900"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18 18 6M6 6l12 12" /></svg></button>
                    </div>
                    <div className="flex-1 overflow-y-auto">
                        {loading ? (
                            <div className="flex flex-col justify-center items-center py-16">
                                <div className="animate-spin rounded-full h-10 w-10 border-b-2 border-[#205781]"></div>
                                <p className="mt-4 text-sm text-gray-700">Loading FAQs...</p>
                            </div>
                        ) : (
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <div className="w-full border border-gray-300 p-2.5 bg-white text-gray-900 rounded">{departmentName}</div>
                                </div>
                                <div>
                                        ...
                                        {faqsList.length > 0 ? (
                                            faqsList.map((item) => (
                                                <div key={item.faq_id} className="flex w-full items-center justify-between p-2">
                                                    <span className="text-gray-800 flex-1 text-sm">{item.question}</span>
                                                    <button type="button" className="text-gray-500 hover:text-red-600 transition duration-200 flex-shrink-0" onClick={() => removeFaq(item.faq_id)}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-5"><path strokeLinecap="round" strokeLinejoin="round" d="M5 12h14" /></svg></button>
                                                </div>
                                            ))
                                        ) : (
                                            <p className="text-sm text-gray-600 text-center py-4">No questions yet</p>
                                        )}
                                    </div>
                                </div>
                            </div>
                    ...
                    <button className="w-full bg-gray-200 hover:bg-gray-300 text-gray-900 transition duration-400 p-2.5 rounded font-medium" onClick={handleClose}>Close</button>
                </div>
            </div>
        </main>
    )
