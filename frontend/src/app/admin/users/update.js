...
export default function Update({ open, close, selectedAccountRow }) {
    ...
    const submit = async () => {
        ...
        if (currentPassword || newPassword || verifyPassword) {
            ...
            if (newPassword && !newPassword.match(/^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[^A-Za-z0-9]).{8,}$/)) {
        ...
        try {
            ...
            if (!res.ok) {
                if (res.status === 409) {
                    ...
                    setUserNameError(data.message || "Invalid input.");
                } else {
                    setUserNameError(data.message || "An unexpected error occurred.");
                }
                return;
            }
